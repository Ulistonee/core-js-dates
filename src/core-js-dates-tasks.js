...
function getWeekNumberByDate(date) {
  ...
  const daysToFirstThursday = (4 - firstDayOfWeek + 7) % 7;
  ...
  const diffInMs = target - firstMonday;
  const diffInDays = Math.floor(diffInMs / (1000 * 60 * 60 * 24));
  const weekNumber = Math.floor(diffInDays / 7) + 1;

  if (weekNumber < 1) {
    return getWeekNumberByDate(new Date(target.getFullYear() - 1, 11, 31));
  }

  if (weekNumber > 52) {
    const yearEnd = new Date(target.getFullYear(), 11, 31);
    const yearEndWeek = getWeekNumberByDate(yearEnd);
    if (weekNumber > yearEndWeek) {
      return 1;
    }
  }
